...
void ACPP_GhostEntity::Fly(const FInputActionValue& _value)
{
	...
	LaunchCharacter(FVector(0.0f, 0.0f, _movementValue) * heightJump, false, true);
	
}
void ACPP_GhostEntity::SwapGameMode(const FInputActionValue& _value) //f
{
	DebugText("swap");
	UWorld* _world = GetWorld();
	if (_world)
	{
		AWorldSettings* worldSettings = _world->GetWorldSettings();
		TSubclassOf<AGameModeBase> _alternateGameModeClass = APlayerGameModeBase::StaticClass();
		worldSettings->DefaultGameMode = _alternateGameModeClass;
		DebugText("success");
	}
}
...
void ACPP_GhostEntity::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	_inputCompo->BindAction(inputToSwapGameMode, ETriggerEvent::Completed, this, &ACPP_GhostEntity::SwapGameMode);
